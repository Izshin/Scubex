    ...
    <AnimatePresence>
      ...
      {!hidden && data && !loading && (() => {
        ...
        <motion.div
          ...
          transition={{ duration: 0.3 }}
        >
          {/* Header: temp + weather icon + minimize + ? button */}
          <div className="flex items-center gap-2">
            ...
            {!minimized && (
              ...
              >
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-x-3 gap-y-2 text-[13px] mt-2">
                  ...
                  <Stat label="Corriente" value={fmt(data.oceanCurrentVelocity, 'km/h')} color={statColor('Corriente', data)} />
                  <Stat label="Swell" value={fmt(data.swellWaveHeight, 'm')} color={statColor('Swell', data)} />
                  <Stat label="Periodo" value={fmt(data.wavePeriod, 's')} color={statColor('Periodo', data)} />
                  <Stat label="Prob. lluvia" value={fmt(data.precipitationProbability, '%', 0)} color={statColor('Prob. lluvia', data)} />
                  <Stat label="Nivel mar" value={fmt(data.seaLevelHeight, 'm')} color={statColor('Nivel mar', data)} />
                </div>

                {/* Forecast strip — next 7 days */}
                {forecast && forecast.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-gray-100/80">
                    <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Próximos días · toca un día</p>
                    <div className="flex gap-1 overflow-x-auto pb-0.5 select-none" style={{ scrollbarWidth: 'none' }}>
                      {forecast.map((day, i) => {
                        const date = new Date(day.date + 'T12:00:00');
                        const isToday = i === 0;
                        const label = isToday ? 'Hoy' : date.toLocaleDateString('es-ES', { weekday: 'short' });
                        const { icon } = getWeatherInfo(day.weatherCode);
                        const isSelected = selectedDayIdx === i;
                        const condDot = day.divingCondition === 'good' ? 'bg-emerald-400' : day.divingCondition === 'moderate' ? 'bg-amber-400' : 'bg-red-400';
                        return (
                          <button
                            key={day.date}
                            onClick={() => setSelectedDayIdx(isSelected ? null : i)}
                            className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl min-w-[46px] flex-shrink-0 transition-all ${
                              isSelected
                                ? 'bg-cyan-100 border border-cyan-300/70 shadow-sm scale-[1.04]'
                                : isToday
                                ? 'bg-cyan-50/80 border border-cyan-200/50 hover:bg-cyan-100/60'
                                : 'bg-gray-50/70 border border-transparent hover:bg-gray-100/80'
                            }`}
                          >
                            <span className="text-[10px] text-gray-500 font-medium capitalize">{label}</span>
                            ...
                            <div className={`w-1.5 h-1.5 rounded-full mt-0.5 ${condDot}`} />
                          </button>
                        );
                      })}
                    </div>
                    ...
                    {/* Expanded day detail */}
                    <AnimatePresence>
                      {selectedDayIdx !== null && forecast[selectedDayIdx] && (() => {
                        const d = forecast[selectedDayIdx];
                        const { desc, icon } = getWeatherInfo(d.weatherCode);
                        const condStyles = getConditionStyles(d.divingCondition);
                        const date = new Date(d.date + 'T12:00:00');
                        const dateLabel = selectedDayIdx === 0
                          ? 'Hoy'
                        ...
                        return (
                          <motion.div
                            ...
                            className="overflow-hidden"
                          >
                            <div className={`mt-2 rounded-xl p-2.5 border ${condStyles.bg} ${condStyles.border}`}>
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-1.5">
                                  <span className="text-xl leading-none">{icon}</span>
                                  <div>
                                    <p className="text-[11px] font-bold text-gray-800 capitalize">{dateLabel}</p>
                                    <p className="text-[10px] text-gray-500">{desc}</p>
                                  </div>
                                </div>
                                <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full whitespace-nowrap ${condStyles.badge}`}>{condStyles.label}</span>
                              </div>
                              <div className="grid grid-cols-3 gap-x-3 gap-y-1.5">
                                <ForecastStat label="Temp. máx" value={d.tempMax !== null ? `${Math.round(d.tempMax)}°C` : '—'} />
                                <ForecastStat label="Temp. mín" value={d.tempMin !== null ? `${Math.round(d.tempMin)}°C` : '—'} />
                                <ForecastStat label="Prob. lluvia" value={d.precipProbMax !== null ? `${Math.round(d.precipProbMax)}%` : '—'} />
                                <ForecastStat label="Viento máx" value={d.windSpeedMax !== null ? `${Math.round(d.windSpeedMax)} km/h` : '—'} />
                                <ForecastStat label="Oleaje máx" value={d.waveHeightMax !== null ? `${d.waveHeightMax.toFixed(1)} m` : '—'} />
                              </div>
                            </div>
                          </motion.div>
                        );
          ...
          >
            ...
            <div className="p-4 border-b border-gray-100 flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Scubex se encarga de proporcionarte datos climáticos interesantes para el buceo. Para el punto que has seleccionado tenemos estas propiedades:
                </p>
              </div>
              <button
            ...
            <div className="overflow-auto p-4 space-y-3 custom-scrollbar">
              ...
              <div className="mt-4 pt-3 border-t border-gray-100">
                ...
                <div className="space-y-1.5 text-xs">
                  <div className="grid grid-cols-4 gap-x-3 items-center text-gray-500 font-semibold mb-1">
                    <span></span><span className="text-emerald-600 text-center">Bueno</span><span className="text-amber-600 text-center">Aceptable</span><span className="text-red-600 text-center">Adverso</span>
                  </div>
                  <div className="grid grid-cols-4 gap-x-3 items-center">
                    <span className="font-medium text-gray-700">Viento</span><span className="text-emerald-600 text-center">&lt; 15 km/h</span><span className="text-amber-600 text-center">15–30 km/h</span><span className="text-red-600 text-center">&gt; 30 km/h</span>
                  </div>
                  <div className="grid grid-cols-4 gap-x-3 items-center">
                    <span className="font-medium text-gray-700">Oleaje</span><span className="text-emerald-600 text-center">&lt; 0.5 m</span><span className="text-amber-600 text-center">0.5–1.5 m</span><span className="text-red-600 text-center">&gt; 1.5 m</span>
                  </div>
                  <div className="grid grid-cols-4 gap-x-3 items-center">
                    <span className="font-medium text-gray-700">Visibilidad</span><span className="text-emerald-600 text-center">&gt; 10 km</span><span className="text-amber-600 text-center">2–10 km</span><span className="text-red-600 text-center">&lt; 2 km</span>
                  </div>
                  <div className="grid grid-cols-4 gap-x-3 items-center">
                    <span className="font-medium text-gray-700">Corriente</span><span className="text-emerald-600 text-center">&lt; 1 km/h</span><span className="text-amber-600 text-center">1–3 km/h</span><span className="text-red-600 text-center">&gt; 3 km/h</span>
                  </div>
                  <div className="grid grid-cols-4 gap-x-3 items-center">
                    <span className="font-medium text-gray-700">Temp. Mar</span><span className="text-emerald-600 text-center">&gt; 20 °C</span><span className="text-amber-600 text-center">15–20 °C</span><span className="text-red-600 text-center">&lt; 15 °C</span>
                  </div>
                  <div className="grid grid-cols-4 gap-x-3 items-center">
                    <span className="font-medium text-gray-700">Swell</span><span className="text-emerald-600 text-center">&lt; 0.5 m</span><span className="text-amber-600 text-center">0.5–1.5 m</span><span className="text-red-600 text-center">&gt; 1.5 m</span>
                  </div>
                  <div className="grid grid-cols-4 gap-x-3 items-center">
                    <span className="font-medium text-gray-700">Prob. lluvia</span><span className="text-emerald-600 text-center">&lt; 20%</span><span className="text-amber-600 text-center">20–60%</span><span className="text-red-600 text-center">&gt; 60%</span>
                  </div>
                </div>
                <p className="text-[11px] text-gray-400 mt-3 leading-relaxed text-center">
                  La condición general se calcula con una media ponderada de estos factores. Oleaje, corriente y visibilidad tienen mayor peso. Si algún factor crítico alcanza valores extremos (oleaje &gt; 2.5 m, viento &gt; 40 km/h, corriente &gt; 5 km/h o tormentas), la condición se marca como adversa automáticamente.
                </p>
              </div>
            </div>

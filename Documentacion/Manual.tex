...
Cuando el escaneo termina, el panel de especies se vuelve desplegable y al abrirse muestra los resultados. El panel del clima tambien lo es, puesto que siempre peude volver a recuperarse el espacio del mapa (Fig.~\ref{fig:escaneoTodoAbierto}).

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/manual/EscaneoTodoAbierto.png}
\caption{Vista completa tras el escaneo: condiciones a la izquierda y especies a la derecha\label{fig:escaneoTodoAbierto}}
\end{figure}
...
% TODO: Crear una publicación: modo Publicación, clic en mapa, formulario (título, descripción, foto), envío.
% TODO: Ver publicaciones ajenas: marcadores en el mapa, panel de detalle.
% TODO: Interacciones: like/unlike, comentarios con @menciones, guardado, compartir.
% TODO: Gestión propia: editar y eliminar publicación.
% TODO: Notificaciones: tipos (like, comentario, mención, follow), campana, navegación desde notificación al punto del mapa.
...
\label{sec:manual-casos}
% ─────────────────────────────────────────────────────────────────────────────

\subsection{Explorador: uso centrado en información de la zona}
\label{sec:caso-explorador}

% TODO: Usuario que busca un lugar, escanea la fauna, consulta las condiciones climáticas y decide si bucear.
% TODO: Flujo: PlaceSearch → clic en mapa → ajuste de radio → escaneo → revisión de especies → revisión del panel climático.

\subsection{Social: uso centrado en la comunidad}
\label{sec:caso-social}

% TODO: Usuario que entra a ver publicaciones recientes, interactúa (likes, comentarios con menciones), sigue a un buceador, revisa notificaciones y guarda publicaciones.
% TODO: Flujo: mapa con marcadores → detalle → like → comentario con @mención → perfil del autor → follow → campana de notificaciones → guardados en perfil.

\subsection{El usuario avanzado: todos los microsistemas en marcha}
\label{sec:caso-avanzado}

% TODO: Sesión en la que confluyen varios sistemas simultáneamente.
% TODO: Ilustrar: caché de especies (segunda consulta en la misma zona es instantánea), actualización optimista de likes (contador sube antes de que el servidor confirme), deduplicación de notificaciones (un comentario con mención no genera dos avisos), polling de notificaciones en segundo plano mientras se navega, geocodificación inversa al publicar.
% TODO: Objetivo: mostrar que hay más cosas funcionando de las que el usuario percibe.

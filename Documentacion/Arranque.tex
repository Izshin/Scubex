...
\begin{enumerate}
    ...
    \item \textbf{Datos Climáticos y Oceanográficos}: Integración de viento (WeatherAPI), temperatura del agua y corrientes (Stormglass).
...
\begin{itemize}
    ...
    \item El escáner de especies constituye el MVP funcional mínimo que justifica el proyecto.
    \item Las características climáticas y sociales son extensiones que añaden valor incremental sin modificar el núcleo arquitectónico.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Frontend}: React 18 + Vite. Single Page Application (SPA) con Hot Module Replacement (HMR) para desarrollo ágil.
...
\begin{itemize}
    \item \textbf{Renderizado vectorial nativo}: Permite animaciones CSS personalizadas (efecto sonar del escáner) sin sobrecarga de DOM.
...
Stormglass&Stormglass.io& Datos oceanográficos: temperatura del agua, altura de olas, corrientes. \textbf{Rate limit}: 10 req/día (uso restringido).\\
	\end{coolTable}
...
\begin{itemize}
    ...
    \item \textbf{WeatherAPI}: Polling cada 30 minutos para actualizar capa de viento en el mapa.
    \item \textbf{Stormglass}: Uso condicional (solo si el usuario solicita datos oceanográficos avanzados).
\end{itemize}
...
\begin{itemize}
    ...
    \item Gestión de timeout en peticiones a Stormglass (timeout: 5s, fallback: omitir datos oceanográficos).
\end{itemize}
